...
def randomize_camera(
    ...
    z += z_add
            ...
            # # Rotate the camera down slightly
            # cam.rotation_euler.x -= 0.1
...
def normalize_scene(cur_obj=None, adjustment=Vector(np.array([0, 0, 0]))) -> None:
    ...
    if cur_obj is None:
        ...
        for obj in get_scene_root_objects():
            ...
            obj.matrix_world.translation += adjustment
        bpy.ops.object.select_all(action="DESELECT")

        # unparent the camera
        bpy.data.objects["Camera"].parent = None
    else:
        bbox_min, bbox_max = scene_bbox(cur_obj)
        print(f"!!bbox_min: {bbox_min}, bbox_max: {bbox_max}")
        if sum(bbox_min - bbox_max) == 0:
            scale = 0.2
        else:
            scale = 1 / max(bbox_max - bbox_min)
        cur_obj.scale = cur_obj.scale * scale
        ...
        offset = -(bbox_min + bbox_max) / 2
        # print(f"!!bbox_min: {bbox_min}, bbox_max: {bbox_max}")
        # offset = -(bbox_min + bbox_max) 
        cur_obj.matrix_world.translation += offset
        cur_obj.matrix_world.translation += adjustment
...
# write a function that find the first children of the object that has animation_data and return that child object
def find_first_child_with_animation(obj):
    for child in obj.children:
        if child.animation_data is not None:
            return child
        else:
            return find_first_child_with_animation(child)


def find_first_node_with_name(obj, name):
    for child in obj.children:
        if name in child.name:
            return child
        else:
            return find_first_node_with_name(child, name)
...
def render_object(
    ...
    # reset the current active object to the first children with animation
    bpy.context.view_layer.objects.active = find_first_child_with_animation(bpy.context.active_object)
    # bpy.context.view_layer.objects.active = find_first_node_with_name(bpy.context.active_object, "SceneRootNode")

    # check the root objects
    # for obj in get_scene_root_objects():
    #     print(f"!!!root objects {obj.name}!!!")
    #     print(a)
    # check the scene box
    # print(f"!!!cur obj scene bbox {scene_bbox(bpy.context.active_object)}!!!")
    # print()
    # print(f"!!!vanilla scene bbox {scene_bbox()}!!!")
    # print(a)
    ...
    # normalize the scene
    # normalize_scene()
    adjustment = Vector(np.array([0, 0, -0.1]))
    # adjustment = Vector(np.array([0, 0, -0.0]))
    normalize_scene(bpy.context.active_object, adjustment=adjustment)
    ...
    for f_idx, frame in enumerate(frames):
        ...
        # anim_obj = find_first_node_with_name(root_obj, "Cube.001_4")
        # anim_obj = find_first_child_with_animation(root_obj)
        print(f"!!!!setting frame to {frame}!!!!")
        bpy.context.scene.frame_set(int(frame))

        # evaluated_matrix = anim_obj.animation_data.action.fcurves[0].evaluate(int(frame))
        # print(f"!!!!evaluated_matrix {evaluated_matrix}!!!!")
        # anim_obj.matrix_world = anim_obj.animation_data.action.fcurves[0].evaluate(int(frame))


        state = "start" if f_idx == 0 else "end"

        cur_save_dir = os.path.join(output_dir, f"{state}/")
        os.makedirs(cur_save_dir, exist_ok=True)

        # root_obj.matrix_world = anim_obj.matrix_world
        # render the images
        # out_data = {
        #     'focal': cam.data.lens,
        # }
        ...
        for i in range(num_renders):
            ...
            # render the image
            render_path = os.path.join(cur_save_dir, f"{data_split}/", f"{i:03d}.png")
            scene.render.filepath = render_path
            bpy.ops.render.render(write_still=True)

            # save camera RT matrix
            # rt_matrix = get_3x4_RT_matrix_from_blender(camera)
            # rt_matrix_path = os.path.join(output_dir, f"{state}_{i:03d}.npy")
            # np.save(rt_matrix_path, rt_matrix)

            frame_data = {
                # 'file_path': f"./{data_split}/{i:03d}.png",
                'file_path': f"./{data_split}/{i:03d}",
                'transform_matrix': listify_matrix(cam.matrix_world)
            }
            out_data['frames'].append(frame_data)
        with open(os.path.join(cur_save_dir, f"transforms_{data_split}.json"), 'w') as out_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="The frames to render the object.",
    )
